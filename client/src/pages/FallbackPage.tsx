...
function RouterSettingsSection() {
  const { t } = useI18n()
  const queryClient = useQueryClient()

  const { data } = useQuery<{ probeOnCooldown: boolean; strictChain: boolean }>({
  ...
  const save = useMutation({
    mutationFn: (body: { probeOnCooldown?: boolean; strictChain?: boolean }) =>
      apiFetch<{ probeOnCooldown: boolean; strictChain: boolean }>('/api/settings/router', {
        method: 'PUT',
  ...
  return (
    <section className="rounded-3xl border bg-card p-4 sm:p-5">
      <div className="flex items-start justify-between gap-4 mb-3">
        <div>
          <h2 className="text-sm font-medium">{t('router.title')}</h2>
          <p className="text-xs text-muted-foreground mt-1">{t('router.description')}</p>
        </div>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="flex items-start gap-3 rounded-2xl border bg-background/40 p-3">
          ...
          <label htmlFor="router-probe-on-cooldown" className="space-y-0.5 cursor-pointer">
            <span className="text-xs font-medium block">{t('router.probeTitle')}</span>
            <span className="text-[11px] text-muted-foreground block">{t('router.probeDescription')}</span>
          </label>
        </div>
        ...
        <div className="flex items-start gap-3 rounded-2xl border bg-background/40 p-3">
          ...
          <label htmlFor="router-strict-chain" className="space-y-0.5 cursor-pointer">
            <span className="text-xs font-medium block">{t('router.strictTitle')}</span>
            <span className="text-[11px] text-muted-foreground block">{t('router.strictDescription')}</span>
          </label>
        </div>
      </div>

      {save.isError && (
        <p className="text-destructive text-xs mt-2">{(save.error as Error).message}</p>
      )}
    </section>
  )
}
