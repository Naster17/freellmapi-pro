...
responsesRouter.post('/responses', async (req: Request, res: Response) => {
  ...
  await runFallbackLoop({
    ...
    route: () => routeRequest(estimatedTotal, state.skipKeys.size > 0 ? state.skipKeys : undefined, preferredModel, false, wantsTools, state.skipModels.size > 0 ? state.skipModels : undefined, groupChain, undefined, undefined, completionOpts.response_format !== undefined),
    dispatch: async (route, attempt) => {
      ...
      logRequest(route.platform, route.modelId, route.keyId, 'success', uncachedPromptTokens(result.usage), completionTokens, Date.now() - start, null, null, null, null, usageCachedTokens(result.usage));
      return 'done';
    },
