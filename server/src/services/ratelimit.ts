...
export function reserveKeySlot(platform: string, keyId: number): void {
  ...
  const stack = slotLeaseIds.get(mapKey) ?? [];
  stack.push(leaseId);
  slotLeaseIds.set(mapKey, stack);
}
...
export function releaseKeySlot(platform: string, keyId: number): void {
  ...
  const leaseId = stack.pop();
  if (stack.length === 0) slotLeaseIds.delete(mapKey);
  if (leaseId !== undefined) releaseLease(leaseId);
}
...
function persistCooldown(
  ...
  withDb(db => {
    db.prepare(`
      INSERT INTO rate_limit_cooldowns (platform, model_id, key_id, expires_at_ms, source, set_at_ms, reason)
      VALUES (?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(platform, model_id, key_id)
      DO UPDATE SET expires_at_ms = excluded.expires_at_ms,
                    source = excluded.source,
                    set_at_ms = excluded.set_at_ms,
                    reason = COALESCE(excluded.reason, rate_limit_cooldowns.reason)
    `).run(platform, modelId, keyId, expiresAtMs, source, setAtMs, reason);
  });
}
...
export function setCooldown(
  ...
  persistCooldown(platform, modelId, keyId, expiresAtMs, source, now, reason ?? null);
}
