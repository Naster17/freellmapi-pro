...
export function up(db: Db): void {
  db.prepare(`
    ...
    VALUES ('opencode', 'hy3-free', 'Hy3 Free (OpenCode Zen)', 6, 4, 'Medium', 20, 200, null, null, '~500M', 262144, 0, 0, 1)
  `).run();
  db.prepare(
    "UPDATE models SET enabled = 0 WHERE platform = 'opencode' AND model_id = 'hy3-free'",
  ).run();
  db.prepare(`
    INSERT OR IGNORE INTO fallback_config (model_db_id, priority)
    SELECT id, 9999 FROM models WHERE platform = 'opencode' AND model_id = 'hy3-free'
...
export function down(db: Db): void {
  db.prepare(
    "UPDATE models SET enabled = 1 WHERE platform = 'opencode' AND model_id = 'hy3-free'",
  ).run();
}
