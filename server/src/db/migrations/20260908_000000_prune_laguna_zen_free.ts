...
export function up(db: Db): void {
  db.prepare(`
    ...
    VALUES ('opencode', 'laguna-s-2.1-free', 'Laguna S 2.1 Free (OpenCode Zen)', 6, 4, 'Medium', 20, 200, null, null, '~500M', 1048576, 0, 0, 1)
  `).run();
  db.prepare(
    "UPDATE models SET enabled = 0 WHERE platform = 'opencode' AND model_id = 'laguna-s-2.1-free'",
  ).run();
  db.prepare(`
    INSERT OR IGNORE INTO fallback_config (model_db_id, priority)
    SELECT id, 9999 FROM models WHERE platform = 'opencode' AND model_id = 'laguna-s-2.1-free'
  ...
  const missing = db.prepare(`
    SELECT m.id, f.enabled
     ...
     WHERE m.platform = 'opencode'
       AND m.model_id = 'laguna-s-2.1-free'
       AND pm.id IS NULL
...
export function down(db: Db): void {
  db.prepare(
    "UPDATE models SET enabled = 1 WHERE platform = 'opencode' AND model_id = 'laguna-s-2.1-free'",
  ).run();
}
