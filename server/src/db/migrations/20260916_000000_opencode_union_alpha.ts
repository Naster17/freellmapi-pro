...
export function up(db: Db): void {
  ...
  insert.run('opencode', 'union-alpha', 'Union Alpha Free (OpenCode Zen)', 7, 4, 'Frontier', 20, 200, null, null, '~500M', 262144, 1, 1, 1);

  db.prepare(`
    UPDATE models SET enabled = 1
     WHERE platform = 'opencode' AND model_id = 'union-alpha'
  `).run();

  db.prepare(`
    INSERT OR IGNORE INTO fallback_config (model_db_id, priority)
    SELECT id, 9999 FROM models WHERE platform = 'opencode' AND model_id = 'union-alpha'
  ...
  const missing = db.prepare(`
    SELECT m.id, f.enabled
     ...
     WHERE m.platform = 'opencode' AND m.model_id = 'union-alpha' AND pm.id IS NULL
